...
1. **パラメータを編集**: `config/parameters.yaml` を開いて以下を入力:
   - 各マシンのIPアドレス (Companionと同じPCのアプリは `127.0.0.1`)
   - アプリ→マシンの割り当て
   - 接続固有の設定 (ProPresenterのポートとパスワード、OBSパスワードなど)
...
3. **Companionにインポート**:
   - **Import/Export** タブに移動
   - **Import** をクリック → `output/church-config.companionconfig` を選択
   - **Full Import** を選択
   - **Connections** タブで接続が緑色になることを確認

インポート後はフェーズ5に進んでください。
